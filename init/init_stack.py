...
class InitStack(core.Stack):
    ...
    def getOrCreateVPC(self):
        ...
        if(self.node.try_get_context('use_default_vpc')=='1'):
            return ec2.Vpc.from_lookup(self,vpc_name=vpc_name,is_default=true) 
        else:
            if(self.node.try_get_context("use_default_vpc")):
                return ec2.Vpc.from_lookup(self,vpc_name=vpc_name,vpc_id=self.node.try_get_context("use_default_vpc"))
            else:
                return ec2.Vpc(self,
                    vpc_name=vpc_name,
                    cidr=vpc_cidr,
                    ...
                    subnet_configuration=
                        [ec2.SubnetConfiguration(name="PublicDMZ",subnet_type=ec2.SubnetType.PUBLIC,cidr_mask=24),
                        ec2.SubnetConfiguration(name="PrivateServices",subnet_type=ec2.SubnetType.PRIVATE,cidr_mask=24)])
